...
% Last Modified by GUIDE v2.5 30-Apr-2013 16:24:22
...
if simData==1 || DA.ConnectServer('Local')>0  %Checks to see if there is an OpenProject and Workbench load
    ...
    if simData==1 || DA.GetSysMode>1 % Checks to see if the Project is running
        if simData==1 || not(isnan(DA.ReadTargetVEX(dproj_name{1},0,10,'F32','F64')))   %checks to see if it loaded the correct project
            ...
            %%
            %Calculate baseline
            if ~isempty(handles.inputBaselineFile) & simData~=1
                load(get(handles.inputBaselineFile,'String'));
                medians = squeeze(medians(:,:,1));
            else 
                if isempty(handles.inputBaselineFile) & simData~=1                
                    [averages, stdevs, medians,baseline]=calculate_baseline(time2collectBaseline, ...
           ...
           while (simData==1 || DA.GetSysMode>1) & good_event_count<50
                tic
                %%
                %Retrieve data
                ...
                if simData~=1;
                    [LogNewData,ANNewData,index,logGdat,analog_dat,ANAcqPos,AcqPos,bufferCounter]=getTDTData(handles,time2collectBaseline,sampling_rate,number_of_sec2read,ii,matlab_loopCounter,bufferCounter);
                ...
                DataAfterCAR(:,:,finalPos:finalPos+size(LogNewData,3)-1)=LogNewData;
                %DataAfterCAR2=cat(3,DataAfterCAR2, LogNewData);
                ANNewData_finalMAT(:,finalPosA:finalPosA+size(ANNewDataMAT,2)-1)=ANNewDataMAT(:,1:size(ANNewDataMAT,2));
                %d=[ d ecog.data];
                if finalPos==1
                    specTimes(finalPos:finalPos+size(LogNewData,3)-1)=ecogSTFT.time;
                else
                    specTimes(finalPos:finalPos+size(LogNewData,3)-1)=ecogSTFT.time+(ii-1);
                end
                ...
                if detected_num_events>0
                    ...
                    if  new_plot_flag==1 & newEventFlag==1
                        ...
                        if ~isempty(get(handles.inputMRIFile,'String'))
                            ...
                            %flattened = reshape_3Ddata(to_plot, window_around_event,size(to_plot,2), to_plot_grid);
                            %to_plot=flattened;
                            handles=plotMRI(handles,'update',to_plot,[],sampPerSecond);
                        end
                    end
...
function [averages, stdevs, medians,logBaselineDataMAT]=calculate_baseline(time2collectBaseline, ...
...
ecog=loadHTKFile_Name([get(handles.htkFilePath,'String') filesep 'Downsampled400'],filenames,[(ii-1)*number_of_sec2read+.001 ii*number_of_sec2read]*1000);
%ecog=downsampleEcog(ecog,800,ecog.sampFreq);
analog=loadHTKFile_Name([get(handles.htkFilePath,'String') filesep 'Analog'],{'ANIN1','ANIN2','ANIN3','ANIN4'},[(ii-1)*number_of_sec2read+.001 ii*number_of_sec2read]*1000);
...
function  [LogNewData,ANNewData,index,logGdat,analog_dat,ANAcqPos,AcqPos,bufferCounter]=getTDTData(handles,time2collectBaseline,sampling_rate,number_of_sec2read,ii,matlab_loopCounter,bufferCounter);
...
function handles=plotMRI(handles,flag,ZscoreEventRelatedAvg,filePath,sampPerSecond)
...
function ecogOut=calcSTFT(data,sampFreq)
...
[YY,tt,ff]=stft_multi_ECoG(data',sampFreq);
ecogOut.data=abs(YY);
ecogOut.time=tt;
ecogOut.freq=ff;
...
if ~isempty(get(handles.inputMRIFile,'string'))
    handles=plotMRI(handles,'initiate',to_plot,get(handles.inputMRIFile,'string'));
end